class ContactInformation < ApplicationRecord 
  ...
  validates :password, length: {minimum: 10}, allow_blank: false, if: Proc.new {|c| c.contact_type == "user"} 
  enum contact_type: {guest:0 , user: 1}
  

  before_validation :add_empty_password, if: Proc.new {|c| c.contact_type == "guest"}
